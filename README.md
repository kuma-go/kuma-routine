...
초등학생 아이와 학부모가 **하루 일정 · 준비물 · 할 일 · 보상**을 함께 관리하는 모바일 앱 프로토타입입니다.
...
- 화면에 나오는 이름·일정·광고는 전부 데모용 가상 데이터입니다.
- 드로어의 `데모 초기화`로 저장된 상태를 지울 수 있습니다.
...
- **☰ → 부모 모드로 보기** (데모 PIN `1234`, 건너뛰기 가능) 로 전환하면 프로필 칩이 생겨 아이와 부모의 일정을 오갈 수 있습니다.
- 일정 카드를 **길게 누르면** 드래그로 시간을 옮길 수 있고, 펼친 뒤 `⋯` 로 메뉴를 엽니다.
- 가족 탭의 QR은 실제로 스캔됩니다. 카메라로 찍어보세요.

---

## 상태

디자인 검증용 프로토타입입니다. 백엔드, 계정, 실제 푸시 알림, 결제는 구현 대상이 아닙니다.
