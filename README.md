...
- 서버 통신, 외부 스크립트, 애널리틱스, 쿠키를 **일절 사용하지 않습니다**.
...
이 앱은 **계정 서버 없이 기기 안(localStorage)에서만** 동작합니다.
그래서 초대 링크를 보내도 상대 기기로 일정이 자동으로 넘어가지 않습니다.
대신 상태를 문자열로 주고받는 방식을 넣었습니다.

1. 보내는 사람: `메뉴 → 가족과 함께 쓰기` 에서 **가족 코드 복사**
2. 카톡 등으로 코드 전달
3. 받는 사람: 같은 화면에 **붙여넣고 가져오기**

가져오면 그 기기의 일정·할 일이 받은 내용으로 바뀝니다.
한쪽에서 고친 내용은 코드를 다시 보내야 반영됩니다.
실시간 동기화가 필요하면 계정 서버가 있어야 합니다.
...
프런트엔드 단독으로 동작합니다. 계정 서버, 기기 간 동기화, OS 푸시 알림, 결제는
아직 붙어 있지 않습니다. 알림은 앱이 열려 있는 동안 인앱 배너로 동작합니다.
