...
App.editSchedule=function(id,presetStart){
  ...
  this.sheet(id?'일정 수정':'새 일정',`
    ...
    ${id?'':`<div class="field">
      <label>반복할 요일 <span id="fDaysCnt" style="color:var(--indigo);font-weight:800">· 1일 선택됨</span></label>
      <div class="swatches" id="fDays">${DAYS.map((x,i)=>`<button class="daychip ${i===this.day?'on':''}" data-d="${i}" aria-pressed="${i===this.day}">${x[0]}</button>`).join('')}</div>
      <p style="margin:8px 2px 0;font-size:11.5px;font-weight:700;color:var(--muted)">여러 요일을 골라 한 번에 만들 수 있어요</p>
    </div>`}
    <div class="field">
      <label>색상</label>
  ...
  `,(b,f)=>{
    ...
    const cntEl=b.querySelector('#fDaysCnt');
    const syncDays=()=>{
      days=[...b.querySelectorAll('#fDays .daychip.on')].map(x=>+x.dataset.d);
      if(cntEl)cntEl.textContent=days.length?`· ${days.length}일 선택됨`:'· 선택된 요일 없음';
      if(cntEl)cntEl.style.color=days.length?'var(--indigo)':'var(--muted)';
    };
    b.querySelectorAll('#fDays .daychip').forEach(s=>s.onclick=()=>{
      s.classList.toggle('on');
      s.setAttribute('aria-pressed',s.classList.contains('on'));
...
App.shareSheet=function(){
  ...
  this.sheet('가족과 함께 쓰기',`
    ...
    <div class="field">
      ...
      <textarea class="inp" id="shCode" readonly style="height:96px;font-family:'SFMono-Regular',Menlo,monospace;font-size:10.5px;line-height:1.5;word-break:break-all">${esc(code)}</textarea>
    </div>
    <button class="btn full" id="shCopy" style="margin-bottom:18px">가족 코드 복사하기</button>

    <div class="field">
      <label>받은 코드 붙여넣기</label>
      <textarea class="inp" id="shIn" placeholder="KUMA1: 로 시작하는 코드를 붙여넣어 주세요" style="height:76px;font-size:12px"></textarea>
    </div>
    <div id="shInfo" style="font-size:12.5px;font-weight:700;color:var(--muted);margin:-6px 2px 14px;min-height:18px"></div>
    <button class="btn full line" id="shApply">코드로 가져오기</button>

    <p style="margin:18px 2px 0;font-size:11.5px;font-weight:700;color:var(--muted);line-height:1.6">
      가져오면 이 기기의 일정과 할 일이 <b>받은 내용으로 바뀝니다.</b><br>
      한쪽에서 수정한 내용은 다시 코드를 보내야 반영돼요.
    </p>`,
    `<button class="btn line full" id="shClose">닫기</button>`,
    (b,f)=>{
      b.querySelector('#shCopy').onclick=()=>{
        const ta=b.querySelector('#shCode'); ta.select();
        const done=()=>this.toast('가족 코드를 복사했어요 · 카톡으로 보내보세요');
        if(navigator.clipboard&&navigator.clipboard.writeText)
          navigator.clipboard.writeText(code).then(done).catch(()=>{try{document.execCommand('copy');done()}catch(e){this.toast('복사에 실패했어요')}});
        else {try{document.execCommand('copy');done()}catch(e){this.toast('복사에 실패했어요')}}
      };
      const info=b.querySelector('#shInfo'), inp=b.querySelector('#shIn');
      let parsed=null;
      inp.oninput=()=>{
        const v=inp.value.trim();
        if(!v){info.textContent='';info.style.color='var(--muted)';parsed=null;return;}
        const r=this.importCode(v);
        parsed=r.ok?r:null;
        info.textContent=r.ok
          ? `가족 ${r.members}명 · 일정 ${r.schedules}개 · 할 일 ${r.todos}개`
          : r.msg;
        info.style.color=r.ok?'#1E7A50':'#D8453A';
      };
      b.querySelector('#shApply').onclick=()=>{
        if(!parsed){this.toast('먼저 올바른 코드를 붙여넣어 주세요');inp.focus();return;}
        this.applyImported(parsed.data);
        this.closeSheet();
        this.toast(`가족 일정을 가져왔어요 · 일정 ${parsed.schedules}개`);
        if(window.ModSound)ModSound.play('complete');
      };
      f.querySelector('#shClose').onclick=()=>this.closeSheet();
    });
};

/* 초대 링크(?invite=)로 들어온 경우 안내 */
App.checkInvite=function(){
  let code=null;
  try{ code=new URLSearchParams(location.search).get('invite'); }catch(e){}
  if(!code)return;
...
(function init(){
  ...
  extra.textContent=`
    .daychip{position:relative;width:42px;height:42px;border-radius:13px;border:1.8px solid var(--line);
      background:transparent;font-weight:800;font-size:14px;color:var(--muted);
    ...
    /* 다중 선택임을 알리는 체크 */
    .daychip.on::after{content:'';position:absolute;right:-4px;top:-4px;width:17px;height:17px;border-radius:50%;
      background:var(--indigo);box-shadow:0 0 0 2px var(--paper);
    ...
    .daychip.on::before{content:'';position:absolute;right:0.6px;top:1.4px;width:5px;height:9px;z-index:2;
      border:solid #fff;border-width:0 2.2px 2.2px 0;transform:rotate(45deg)}
    #phone.th-dark .daychip{border-color:#3A3A48}
    #phone.th-dark .daychip.on{background:rgba(122,110,234,.16);border-color:#7A6EEA;color:#B7AEFF;box-shadow:inset 0 0 0 1px #7A6EEA}
    #phone.th-dark .daychip.on::after{background:#7A6EEA;box-shadow:0 0 0 2px var(--paper)}
  ...
  App.render();
  if(App.checkInvite)App.checkInvite();
